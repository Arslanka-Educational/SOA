...
class RouteSpecification(private val filter: GetRoutesFilterParameterDto) : Specification<Route> {
    override fun toPredicate(
        ...
        filter.locationIdFrom?.let {
            predicates.add(
                criteriaBuilder.equal(
                    root.get<Location>("from_id").get<Long>("id"),
                    it
                )
            )
        }
        filter.locationIdTo?.let {
            predicates.add(
                criteriaBuilder.equal(
                    root.get<Location>("to_id").get<Long>("id"),
                    it
                )
            )
        }
        ...
        filter.minX?.let {
            predicates.add(
                criteriaBuilder.greaterThanOrEqualTo(
                    root.get<Coordinates>("coordinates").get<Int>("x"), it
                )
            )
        }
        filter.maxX?.let {
        ...
        filter.minCreationDate?.let {
            predicates.add(
                criteriaBuilder.greaterThanOrEqualTo(
                    root.get<java.time.OffsetDateTime>(
                        "creationDate"
                    ), xmlGregorianCalendarToOffsetDateTime(it)
                )
            )
        }
        filter.maxCreationDate?.let {
            predicates.add(
                criteriaBuilder.lessThanOrEqualTo(
                    root.get<java.time.OffsetDateTime>(
                        "creationDate"
        ...
        filter.fromLocationX?.let {
            predicates.add(
                criteriaBuilder.greaterThanOrEqualTo(
                    root.get<Location>("from").get<Int>("x"), it
                )
            )
        }
        filter.toLocationX?.let {
            predicates.add(
                criteriaBuilder.lessThanOrEqualTo(
                    root.get<Location>("to").get<Int>("x"), it
                )
            )
        }
        filter.fromLocationY?.let {
            predicates.add(
                criteriaBuilder.greaterThanOrEqualTo(
                    root.get<Location>("from").get<Int>("y"), it
                )
            )
        }
        filter.toLocationY?.let {
            predicates.add(
                criteriaBuilder.lessThanOrEqualTo(
                    root.get<Location>("to").get<Int>("y"), it
                )
            )
        }
        filter.fromLocationZ?.let {
            predicates.add(
                criteriaBuilder.equal(
                    root.get<Location>("from").get<Long>("z"),
                    it
                )
            )
        }
